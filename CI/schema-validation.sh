...
function copy_files_from_sftp() {
    mkdir -p $files_dir
    scp -tt -o "StrictHostKeyChecking no" -i "$1" -P 22 -r peCPSFmaster@edisaf.deutschepost.de:/peCPSF1* ./"$files_dir" || true
    scp -tt -o "StrictHostKeyChecking no" -i "$1" -P 22 -r peCPSFmaster@edisaf.deutschepost.de:/peCPSF2* ./"$files_dir" || true
}
